...
typedef enum
{
    XMODEM_SERVER_STATE_START,        /**< Initial state, sending NAK to the client and waiting symbol */
    XMODEM_SERVER_STATE_SOH,          /**< Waiting for the SOH character from the client. */
    XMODEM_SERVER_STATE_PACKET_GET,   /**< Receiving the packet from the client. */
...
typedef struct
{
    XModemServerErr_e (*isReceivedByte)(void* server, bool* isRx);                ///< Hooks to check if a byte is available from the xmodem client
    XModemServerErr_e (*transmitByte)(void* server, uint8_t byte);                ///< Hooks to transmit a byte to the xmodem client
    XModemServerErr_e (*receiveByte)(void* server, uint8_t* byte);                ///< Hooks to receive a byte from the xmodem client
    XModemServerErr_e (*delayMs)(void* server, int ms);                           ///< Hooks to delay for a specified time
    XModemServerErr_e (*writeToMemory)(void* server, uint8_t* data, int size);    ///< Hooks to write data from the xmodem client
